...
`csu_Ethernet_cm.c` 내의 `Ethernet_StateMachine()` 함수에 의해 **CM 코어의 100ms(10Hz) 주기** 백그라운드 태스크에서 폴링 구동됩니다.
> Rx 파싱의 경우, 100us 모터 제어 주기와의 간섭을 막기 위해 W6100 `INTn` 외부 인터럽트(XINT1)를 통해 비동기로 파싱 및 ACK 리턴(`csu_Ethernet_cm_Rx_ISR` 성격)을 즉각 수행합니다.
...
## 5. 제어 변수 및 데이터 구조 (`csu_Ethernet_cm.h`)
