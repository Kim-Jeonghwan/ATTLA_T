...
## 21. ATTLA_T (초장사정 자동선회잠금장치) 하드웨어 회로 핀맵 분석

**작성 일자**: 2026. 06. 08.  
**대상 회로도**: 초장사정 자동선회잠금장치 - 제어보드 초안 (4장)

### 21.1 주요 디지털 제어 핀 (GPIO Mux Mapping)
| 기능 | 매크로 / 신호명 | DSP 핀 (GPIO) | 설명 |
|---|---|---|---|
| **모터 구동 (DRV8343)** | `DSP_DRV_ENABLE` | GPIO0 / EPWM1A | DRV8343 Enable |
| | `DSP_DRV_PWM` | GPIO1 / EPWM1B | 1x PWM 모드: PWM 인가 (INHA) |
| | `DSP_DRV_DIR` | GPIO2 / EPWM2A | 1x PWM 모드: 방향 제어 (INLA) |
| | `DSP_DRV_BRAKE` | GPIO3 / EPWM2B | 1x PWM 모드: 브레이크 (INHB) |
| | `DSP_DRV_nFAULT` | GPIO4 | DRV8343 Fault 모니터링 (Active Low) |
| **홀 센서 입력** | `DSP_HALL_A_IN` | GPIO32 | 모터 위치 피드백 A |
| | `DSP_HALL_B_IN` | GPIO33 | 모터 위치 피드백 B |
| | `DSP_HALL_C_IN` | GPIO34 | 모터 위치 피드백 C |
| **W6100 (체계 통신)** | `DSP_W6100_SPIA_nCS` | GPIO14 / SPISTE_A | SPI A 채널 Chip Select |
| | `DSP_W6100_SPIA_SIMO` | GPIO15 / SPISIMO_A | SPI A 채널 MOSI |
| | `DSP_W6100_SPIA_SOMI` | GPIO16 / SPISOMI_A | SPI A 채널 MISO |
| | `DSP_W6100_SPIA_CLK` | GPIO17 / SPICLK_A | SPI A 채널 CLK |
| | `DSP_W6100_INTn` | GPIO18 | W6100 인터럽트 |
| | `DSP_W6100_RSTn` | GPIO19 | W6100 리셋 |
| **엔코더 (BiSS-C)** | `DSP_ENC_DATA` | GPIO24 | BiSS-C 데이터 입력 |
| | `DSP_ENC_CLK` | GPIO25 | BiSS-C 클럭 출력 |
| **상태 LED** | `DSP_LED_nNORMAL` | GPIO확인필요 | NORMAL 상태 표시 LED (Active Low 구동) |
| | `DSP_LED_nFAULT` | GPIO확인필요 | FAULT 상태 표시 LED (Active Low 구동) |
| **리미트/모니터링**| `DSP_nLIMIT1` | GPIO확인필요 | 리미트 스위치 1번 입력 (Active Low) |
| | `DSP_nLIMIT2` | GPIO확인필요 | 리미트 스위치 2번 입력 (Active Low) |
| | `DSP_PM_n24V` | GPIO확인필요 | 24V 브레이크/스위치 전원 감시 (Active Low) |
| **브레이크 출력** | `DSP_BRAKE` | GPIO확인필요 | TLP293 절연 ➡️ P-Ch MOSFET 제어 |
| **FRAM (메모리)** | `FRAM_SPIB_SIMO` | GPIO63 / SPISIMO_B | SPI B 채널 MOSI |
| | `FRAM_SPIB_SOMI` | GPIO64 / SPISOMI_B | SPI B 채널 MISO |
| | `FRAM_SPIB_CLK` | GPIO65 / SPICLK_B | SPI B 채널 CLK |
| | `FRAM_SPIB_nCS` | GPIO66 / SPISTE_B | SPI B 채널 Chip Select |
| **디버그 (RS-422)** | `DSP_DBG_SCIA_TX` | GPIO84 / SCITX_A | SCI A 채널 TX |
| | `DSP_DBG_SCIA_RX` | GPIO85 / SCIRX_A | SCI A 채널 RX |

### 21.2 주요 아날로그 입력 (ADC) 매핑 (정밀 스펙 포함)
| 신호명 | 대상 | DSP ADC 채널 | 변환 스펙 및 비고 |
|---|---|---|---|
| `DSP_ADC_ISEN_MOT` | 모터 부하 전류 | ADCINA2 | TMCS1108 (100mV/A) ➡️ OP-AMP 0.619 Gain (+24A=2.98V, -24A=0.01V) |
| `DSP_ADC_ISEN_BRK` | 브레이크 구동 전류 | ADCINA3 | TMCS1108 ➡️ OP-AMP 0.619 Gain, 10kHz LPF |
| `DSP_ADC_VSEN_28V` | 28V 전원 모니터링 | ADCIN14 | OP-AMP 차동 증폭 1.503 Gain (50V ➡️ 약 2.96V) |
| `DSP_ADC_VSEN_REF` | 4.096V 기준 전압원 | ADC 채널확인 | REF3040 출력 후 OP-AMP 전압 분배 (2.048V) |
| `DSP_ADC_TSEN_BD` | 외부 보드 온도 | ADCIN15 | MAX6605 내장 아날로그 온도 센서 |

### 21.3 하드웨어 아키텍처 특이사항 및 업데이트 내역
1. **DRV8343HPWPRQ1 제어**: 하드웨어 제어 버전(H)으로 SPI 통신 없이 저항으로 1x PWM 모드가 고정되어 있습니다. 단일 PWM, DIR, BRAKE 핀으로 제어합니다.
2. **W6100 제어 (SPI A)**: 하드웨어 TCP/IP 스택을 담당하므로, CM 코어에서 W6100을 직접 제어할 수 있도록 CPU1이 초기화 단계에서 SPI A 핀(14~17) 및 INT/RST(18,19)의 **마스터 제어권을 CM으로 위임(MasterCore 설정)**하는 구조가 권장됩니다.
3. **BiSS-C 엔코더**: GPIO 24/25를 통한 소프트웨어 비트뱅잉 또는 CLB/SPI 활용 프로토콜 구현이 필요합니다.
4. **리미트 및 LED 핀맵 재확인 필요**: 새 회로도에서 추가된 듀얼 LED(`nNORMAL`, `nFAULT`) 및 리미트 스위치(`nLIMIT1`, `nLIMIT2`), 파워모니터(`PM_n24V`), `DSP_BRAKE` 제어 핀의 정확한 GPIO 번호가 전체 핀맵 시트에서 재확인이 필요합니다.
