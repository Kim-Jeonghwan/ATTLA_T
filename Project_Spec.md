...
- **PID 제어 루프 (3-Stage Cascade) 및 제약 (Soft Limit)**:
  ...
  - **위치 제어기 (`posPid`)**: Kp=1.0, Ki=0.0, Kd=0.0 / dt=0.004s (4ms Multi-Rate) / 출력 제한 ±3500.0 RPM (출력값은 목표 속도 지령으로 사용됨)
  - **속도 제어기 (`speedPid`)**: Kp=0.5, Ki=0.01, Kd=0.0 / dt=0.001s (1ms Multi-Rate) / 출력 제한 ±10.0 A (출력값은 목표 전류 지령치로 사용됨)
  - **전류 제어기 (`currPid`)**: Kp=2.0, Ki=0.05, Kd=0.0 / dt=0.0001s / 출력 제한 0.0 ~ 100.0 % (크기(Magnitude) 기반 제어, 출력값은 절대 Duty 비율로 사용됨)
  - **제어 순서**: 위치 지령(제한 적용) ➡️ `posPid` ➡️ 목표 속도 ➡️ `speedPid` ➡️ 목표 전류량 ➡️ `currPid` ➡️ 최종 목표 Duty 도출.
