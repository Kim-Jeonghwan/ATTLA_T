...
| **CPU1 역할** | 모터 제어(1x PWM 듀티/방향 제어), 전체 시스템 상태 머신, 노트북(W6100) 통신 제어 |
| **CM 역할** | 체계 연동 이더넷(PHY) 제어 및 통신 전담 |
...
### 4.1 이더넷 통신망 분리 (PHY 및 W6100 듀얼 채널) 및 연동통제안 규격
본 시스템은 네트워크 부하 분산 및 보안을 위해 **체계(상위제어기) 통신망**과 **노트북(분석/디버깅) 통신망**을 물리적으로 분리하여 듀얼 채널 이더넷 아키텍처로 구성하였습니다. **(PHY 채널은 CM 코어 전담, W6100 채널은 CPU1 전담 제어)**

- **채널 1: 상위제어기 (체계 연동망)**
  - **하드웨어**: 내부 EMAC 및 외부 이더넷 PHY 칩셋(DP83822HFRHBT) 적용. 외부 커넥터(J2) 간 절연 트랜스포머(HX1188NL) 적용.
  - **인터페이스**: MII (Media Independent Interface) 기반 연결.
  - **역할**: 화포통제컴퓨터 등 상위제어기와의 실시간 제어/상태 명령 송수신 전담.

- **채널 2: 노트북 (분석 및 디버깅망)**
  - **하드웨어**: 하드웨어 TCP/IP 스택 기반 이더넷 칩셋(W6100) 적용 (HX1188NL 절연, TJ 커넥터).
  - **인터페이스 (SPI-A)**: `SIMO`(16), `SOMI`(17), `CLK`(18), `nCS`(19), `INTn`(20, XINT1 외부 인터럽트 수신), `RSTn`(21)
  - **역할**: 노트북 연결을 통한 상태 모니터링 및 독립적 오프라인 분석 통신망. **(CPU1 전담 제어, `csu_Debug_cpu1.c` 모듈 및 동적 IP 라우팅 적용)**
  - **프로토콜 스펙**: 상세 규격은 `Debug_Protocol.md` 참조.

- **통신 프로토콜 기본 정책 (상위제어기 연동 기준)**: 
...
| :--- | :--- | :--- | :--- |
| **W6100 (노트북 이더넷)** | **SPI-A** | `POL0PHA0`<br>10 MHz | 클럭 Idle Low, 상승 에지 수신 (SPI Mode 0). 초기 20MHz로 기획되었으나, 시스템 LSPCLK 최대 한계(50MHz / 4 = 12.5MHz) 초과로 인한 Driverlib 초기화 에러(ASSERT)를 방지하기 위해 가장 안정적인 10MHz로 속도를 수정 적용함. |
...
## 6. 코어 간 데이터 교환 및 공유 메모리 아키텍처 (CPU1 ↔ CM)

본 프로젝트는 듀얼 코어(CPU1, CM) 간의 데이터 전송 지연과 IPC 인터럽트 오버헤드를 최소화하기 위해, 목적에 따라 하드웨어 IPC와 Message RAM(MSGRAM)을 분리하여 사용합니다.

### 6.1 부팅 동기화 (단발성 하드웨어 IPC)
- **제어 로직**: 초기 부팅 단계에서 CM 코어가 독자적으로 기동하여 이더넷(PHY) 및 내부 인터럽트 초기화를 마친 후, CPU1에 `IPC_CMD_CM_BOOT_READY` 신호를 발송합니다.
- **목적**: 통신/제어 루프가 완전한 상태로 맞물리도록 하는 핸드셰이크 목적으로만 하드웨어 IPC 인터럽트를 제한적으로 사용합니다.

### 6.2 실시간 데이터 교환 (Message RAM 폴링)
- **배경**: 통상적인 실시간 센싱 데이터(BIT 정보, 센서 온도 등) 전달 시마다 IPC 인터럽트를 발생시키면 코어의 제어 주기에 악영향을 미칩니다. 또한, TMS320F28388D 하드웨어 설계 상 CM 코어는 GSRAM(Global Shared RAM)에 대한 쓰기 권한이 제한되므로, 버스 에러(Hard Fault)를 방지해야 합니다.
- **구현 방식**:
  - 코어 간 실시간 양방향 데이터 전송은 전용으로 할당된 **MSGRAM (Message RAM)** 영역에 구조체 포인터(`pxDataCpu1ToCm`, `pxDataCmToCpu1`)를 매핑하여 수행됩니다.
  - 인터럽트 발생 없이 각 코어가 자신의 할당된 MSGRAM 영역에 비동기적으로 데이터를 기록하고(`Write`), 상대 코어가 필요할 때 주기적으로 해당 영역을 읽어가는(`Read`) **폴링(Polling) 방식**을 채택했습니다.
  - **데이터 찢김 방지 (Lock-Free Seqlock 방식, 홀짝 카운터 로직)**: 인터럽트 지연을 막기 위해 락(Lock)을 사용하지 않는 **Seqlock(Sequence Lock)** 기법을 도입하여 데이터 무결성을 보장합니다. 송신 측이 데이터 쓰기를 시작할 때 카운터(`seqCount`)를 1 증가시켜 **홀수(Odd)** 로 만들고, 쓰기를 완료하면 다시 1 증가시켜 **짝수(Even)** 로 만듭니다. 수신 측은 카운터가 짝수일 때만 데이터 쓰기가 완료되었다고 판단하여 읽어(Read)가며, 읽기 전과 후의 카운터 값을 비교하여 읽는 도중 쓰기 침범(데이터 찢김)이 발생하지 않았는지 철저하게 검증하는 락 프리(Lock-Free) 구조입니다.

---

## 7. 글로벌 구조체 변수 파이프라인 (디버깅 및 제어 상태 참조용)
...
### 7.1 CSU (Control & Service Unit) 계층
...
| **`xEthCtrl`** | `stEthControl` | 상위제어기 이더넷(CM) 통신망 가입 상태 머신, 재전송 타이머, CBIT/IBIT 제어 구조체 |
| **`xDbgCtrl`** | `stDbgControl` | 노트북 디버그(CPU1) 통신망 동적 IP 캡처 및 텔레메트리 자동 송신 제어 구조체 |

### 7.2 HAL (Hardware Abstraction Layer) 계층
...
## 8. 미구현 및 추후 확정 예정 사항 (Pending Implementation & TBD)
